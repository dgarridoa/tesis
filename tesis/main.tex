...
\begin{enumerate}
    \item \textbf{Evolución de los tópicos}: la evolución de los tópicos se refleja en el cambio en la distribución sobre las palabras. Esto permite detectar cambios en cómo se comete un mismo tipo de delito, por ejemplo, el \quotes{portonazo} en un determinado momento se comete en grupos de 2-3 personas con arma blanca, luego evoluciona de arma blanca a arma de fuego y lo perpetran jóvenes menores de edad.
...
Una construcción alternativa de HDP es conocida bajo el nombre de \textit{Chinese Restaurant Franchise Process} (CRF), una extensión del CRP, que permite compartir un conjunto de platos a través de una cadena de restaurantes Chinos. La analogía es la siguiente, tenemos $D$ restaurantes (documentos), cada uno con $N_{d}$ clientes $\phi_{d,i}$ (palabras), que se sientan en tablas $t_{d,i}$, en cada tabla es servido un único plato $\phi_{k}\sim H$ (tópico) a partir de un menú común para todos los restaurantes. \\

Sea $m_{dk}$ el número de tablas sirviendo el plato $k$ en el restaurante $d$, así $m_{d.}$ representa el número de tablas en el restaurante $d$, $m_{.k}$ representa el número de tablas sirviendo el plato $k$, y $m_{..}$ el número total de tablas ocupadas. Primero, procedemos a integrar afuera $G_{d}$, siendo la distribución condicional del cliente $i$ en la tabla $d$ es:

\begin{align}
p(t_{di}=t|t_{d1}, \ldots, t_{j,i-1}, \alpha_{0}, G_{0}) = \frac{1}{\alpha_{0}+i-1}\bigg(\alpha_{0}\mathbb{I}(t=t^{*})+\sum_{t^{'}=1}^{m_{d.}}N_{dt^{'}}\mathbb{I}(t=t^{'})\bigg)
\end{align}

donde $N_{dt^{'}}$ representa los clientes del restaurante $d$ que están sentados en la tabla $t^{'}$. Con probabilidad proporcional a los clientes sentados en la tabla $t$ los clientes del restaurante se sentarán en esta y con probabilidad proporcional a $\alpha_{0}$ en una nueva. Una vez todos los clientes estan sentados tenemos una partición sobre $\phi_{d1}, \ldots, \phi_{dN_{d}}$. Luego, procedemos a integrar afuera $G_{0}$, obteniéndose:
...
Al igual que LDA la distribución posterior es intratable, en  \citep{teh2005sharing} se propone un algoritmo basado en Gibbs Sampling bajo la construcción CRF de un HDP. En este trabajo se utiliza la implementación en C++ \citep{HDP} de este algoritmo.

%\todo[inline][Hiperparámetros]

%Si bien el modelo posee hiperparámetros estos no se calibraron en base alguna métrica. Los parámetros de concentración fueron marginalizados por un prior vague gamma (empirical bayes), distribución que tiene gran parte de su masa en cero y una cola pesada. Como consecuencia de esto la distribución le dara más peso a la ver a los datos que al prior(?). Adicionalmente el modelo tendrá a tener menos tópicos y a clasificar nuevas palabras en tópicos previamente generados (Chinese Restaurant Process). En el caso de la dirichlet se usa n=0.5 /V simétrica, en general se prefiere eta menor que 1 para generar tópicos con distribución sparse, muy distintos, y que la semántica este compacta en pocas palabras para facilitar la interpretabilidad.
...
% se podría meter mano para reemplazar por WMD
