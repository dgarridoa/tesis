...
\todo{Descripción del capitulo y libro de referencia}

En esta sección ..., \citep{orbanz2012lecture}...
...
\label{sec:crp}
\citep{aldous1985exchangeability}
...
\label{sec:hdp}
Hierarchical Dirichlet Process (HDP)\citep{teh2005sharing} es una colección de DP que comparten una distribución base $G_{0}$, la cual además es dibujada a partir de un DP (ver representación gráfica del modelo en la Figura \ref{img:hdp}). Matemáticamente, a nivel corpus se tiene que la distribución base $H \sim Dir(\frac{1}{|V|}1_{|V|})$ y $G_{0} \sim DP(\gamma, H)$, luego, para cada documento $d$ del corpus $D$ se asume que es dibujado por el siguiente proceso generativo:
\begin{enumerate}
